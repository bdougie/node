...
class ModuleJob {
  ...
  async run() {
    ...
    try {
      ...
      if (err.message.includes('is not defined') && (err.stack.includes("esm/loader"))) {
        err.message = 'You cannot require from an ESM module. Replace require with import or leverage createRequire.';
        err.stack = err.stack.replace(/ReferenceError:.*\n/,
                                      `ReferenceError: ${err.message}\n`);
      }
      throw err;
    }
